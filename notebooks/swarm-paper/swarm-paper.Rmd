...
raw_df <- read_csv(paste0("../../data/raw_data.", DATA_VERSION, ".csv"),
                   na=c("", "nan"),
                   col_types=cols(item_id=col_factor(NULL))) %>% 
...
load_json = function(materials_id) {
  ret <- tryCatch(
    {jsonlite::read_json(paste0("../../materials/", materials_id, ".json"),
        ...
        mutate(materials_id=materials_id) %>% 
        type_convert()},
    error=function(err){})
  return(ret)
}
...
df.02 %>%
  # Calculate sd-normalized difference between good/bad fillers per subject
  group_by(uniqueid) %>% 
    summarise(subject_good_bad_contrast=(mean(slider_value[condition_1 == "good"]) - mean(slider_value[condition_1 == "bad"])) / sd(slider_value)) %>% 
  full_join(df.02, by=c("uniqueid")) %>% 
  
  # Join in exclusion information
  full_join(select(subject.ttest.02, uniqueid, exclude), by=c("uniqueid")) %>% 
  
  # Reorder subjects based on sd-normalized difference
  mutate(uniqueid=fct_reorder(uniqueid, -subject_good_bad_contrast)) %>% 
...
```{r 02 regression}
do_acceptability_regression = function(df, contrast_condition) {
  df = df %>% filter(condition_1 %in% c("bad", contrast_condition)) %>% 
    mutate(contrast = condition_1 == contrast_condition)
  ...
  return(list(model=model, df_controlled=df_controlled))
}
lmer.02.A = do_acceptability_regression(df.02, 1)
lmer.02.L = do_acceptability_regression(df.02, 0)
