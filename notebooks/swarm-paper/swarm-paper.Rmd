...
We evaluated the effect of our experimental manipulation using two mixed-effects linear regression models, each independently comparing acceptability ratings on a construction (A-construction or L-construction) to ungrammatical items. The fixed effect of interest was this *Contrast* factor (sum-coded; ungrammatical items = -1, critical item = 1). We included as sum-coded covariates *Countability* (whether the agent/theme was a mass or count noun) and *Animacy* (whether the agent/theme expressed an animate participant). Finally, we included interaction terms between these control variables and the *Contrast* factor.
...
lmer.02.A = xfun::cache_rds({do_acceptability_regression(df.02.filtered, condition_1, 1, "bad")})
lmer.02.L = xfun::cache_rds({do_acceptability_regression(df.02.filtered, condition_1, 0, "bad")})

lmer.02.A.tidy = lmer.02.A$model %>% tidy
lmer.02.L.tidy = lmer.02.L$model %>% tidy

lmer.02.A.print = papaja::apa_print(lmer.02.A$model)
lmer.02.L.print = papaja::apa_print(lmer.02.L$model)
```

```{r 02 regression results, results="asis"}
papaja::apa_table(lmer.02.A.print$table, caption="Acceptability regression results for the A-construction")
papaja::apa_table(lmer.02.L.print$table, caption="Acceptability regression results for the L-construction")
```


In each regression we find a significant main effect separating the constructions of interest from ungrammatical English items (A-construction: `r lmer.02.A.print$statistic$contrast1`; L-construction: `r lmer.02.L.print$statistic$contrast1`).
...
# TODO why are there NA excluded values here? set of subjects should be the same
# in df.00 as in subject.ttest.00
df.00 %>%
