...
## Comprehension as inverse inference

TODO introduce causal forward model

Several popular models of language comprehension explicitly pitch comprehension
at the computational level, as performing a conditional inference on this
causal network.

Noisy-channel inference models, for example, posit that comprehenders carry out
inverse inference over a noisy language production model when determining the
intended message of a speaker [@levy2008noisychannel;@levy2009eye].
Rational speech act (RSA) models likewise posit that comprehenders do inference
over a model of production incorporating a speaker's knowledge state and
available alternative utterances [@goodman2016pragmatic].

Taken to its full extent, however, this claim leads to a tractability problem.
TODO
- need to marginalize over meanings/alternative structures
- pragmatics models get around this by talking about restricted scales in
  meaning space, or small generative algorithms for alternative utterances
- also need to account for possible interactions & explaining-away effects
  across levels of representation
  - positive results here: TODO CITE Myslin CUNY paper
  
In this paper, we examine the particular production phenomenon of construction
choice. The following sections will present the production phenomenon TODO
- after which return to comprehension problem

## Construction choice in production

Natural language affords speakers many degrees of freedom in communicating their
message. For example, English has at least four forms which vary in their
syntactic structure but communicate somewhat synonymous messages:

1. Bees are buzzing in the garden.
2. The garden is buzzing with bees.
3. The garden is abuzz with bees.

This choice of syntactic construction is one among many degrees of freedom
available to the speaker, such as lexical choice (saying *bees* or *insects* or
*buzzing things*) and anaphora and deixis (saying *them* or *those things* to
refer to the bees, or *here* or *there* to refer to the garden; see e.g.
@kehler2008coherence).

We are interested in the phenomenon of construction choice in particular
because speakers take into account factors at multiple levels of representation
when choosing a construction: they use different constructions to signal
differences in intended meaning, but also navigate among structures in a way
that keeps a discourse coherent.
<!-- While other production decisions such as -->
<!-- pronominalization depend on multiple interacting -->
<!-- pressures, the link BLAH BLAH more computationally interesting -->

The following sections present two independent factors which speakers take into
account when choosing a construction: intended meaning and discourse coherence.

### Meaning and construction choice

Speakers use different syntactic constructions to signal their
interpretation of or perspective on an event.
For example, @wolff2003direct
demonstrated with a forced-choice production paradigm that speakers alternate
between a lexical causative (*She opened the door*) and a periphrastic
causative (*She caused the door to open*) depending on properties of the evident
causal link between the agent and patient.

<!-- TODO is this para relevant? -->
Aspects of lexical meaning also constrain speakers' choice of constructions.
@levin1993english demonstrated how many fine-grained meaning contrasts govern
differences in verbs' syntactic distribution. For example, English verbs of
caused motion which do not lexicalize a path of motion cannot appear in a
simple transitive without modification (*\*She put the shelf*) while verbs
which code paths can do so (*She raised/lowered/lifted/dropped the shelf*).

In this paper, we study a minimal pair of constructions known as the *swarm*
alternation, which describe an active relationship between an Agent/Theme and a
Location [@salkoff1983bees;@dowty2001semantic]:

1a. (A-construction) Bees are swarming in the garden.
1b. (L-construction) The garden is swarming with bees.
2a. (A-construction) Debris is flowing in the river.
2b. (L-construction) The river is flowing with debris.

TODO meaning contrast according to Salkoff/Dowty.

TODO why choose *swarm* here?
- meaning contrast is less *a priori* relevant for the syntax-semantics interface
- so it's computationally more impressive if it works.

Our experiments in this paper presented these forms of the *swarm* alternation
to subjects and measured their resulting interpretations. In the following text
we refer to these forms as variants of the *critical sentence*.

### Discourse structure and construction choice
...
For example, @prat-sala2000discourse demonstrated in an image description
paradigm that
subjects varied in their preference for active or passive sentence forms
in a way that allowed them to place discourse-given entities in grammatical
subject position.
...
We accomplished this by combining the critical sentence (A-construction or
L-construction, as described in the previous section) with a prefix sentence
which mentioned either the Agent/Theme or Location of the relevant event:
...
## The challenge for comprehension

TODO
- so we've got two factors here: space of possible meanings and discourse state
- present comprehension as Bayes' Rule here
- point out marginalization difficulty
...
<!-- TODO address complication: syntactic position of entity in first sentence will push for/against pronominalization in the second sentence to a different degree. prior grammatical subjects are likely to be referred to pronominally. see @rohde2014grammatical, @kehler2008coherence -->
...
We designed an experiment testing whether comprehenders' inferences about
speaker meaning reflected the normative pattern of Bayesian inference known
as *explaining away*. Specifically, after confirming that comprehenders were
sensitive to the meaning difference between two syntactic constructions, we
asked whether they would modulate this meaning inference when other 
countervailing pressures for construction choice were evident.

Our results find no significant explaining-away effect in comprehenders'
meaning inferences. This is despite the fact that they clearly recognize
the basic difference in meaning due to construction (confirmed in Section
\@ref(experiment-swarm-comprehension-basic)) and also demonstrate a willingness
to alternate between the two constructions to maintain discourse coherence
(confirmed in Section \@ref(experiment-swarm-production-givenness)).

## Available explanations

There are two broad types of explanation which could admit these results: an
*approximately Bayesian* account, or an *anti-Bayesian* account.

### Approximate Bayesian inference

On the approximately Bayesian account, comprehenders are indeed solving a
problem of Bayesian inverse inference at the computational level, but must make
sacrifices for algorithmic tractability. Many computational cognitive models
make this move, explaining classes of error as resulting from biases in an
approximate inference algorithm [TODO CITE].

<!-- TODO needs a little more hand-holding here to be intuitive for less computational reader. -->
Concretely in this case, we could imagine a sampling-based approximate inverse
inference algorithm producing these results. On this view, speakers possess a
generative model of utterances $P(u \mid m)$ deployed in production. But
deploying this same model in comprehension according to Bayesian norms is
massively intractable. Consider the inverse inference following Bayes' rule:

$$
P(m \mid u) = \frac{P(u \mid m) P(m)}{P(u)} = \frac{P(u \mid m) P(m)}{\sum_{m'} P(u \mid m') P(m')}
$$

Computing this distribution $P(m \mid u)$ thus requires marginalizing over all
possible alternative meanings $m'$. As we demonstrated in the introduction,
however, this normalizing value is just what enables the explaining-away effect.
<!-- TODO say a few more words about why this is intractable -->

A sample-based approximation to this normalizing value might incorporate some
*proposal function* $q(m \mid C)$, which proposes potential meanings based on
features of
the context $C$. This proposal function can be structurally and computationally
simpler than the actual normative model $P(m \mid u)$.
Given minimal assumptions about the behavior of $q$ hold, we
can approximate this normalizing value $P(u)$ by drawing proposals of
$m$ from this model and reweighting them according to the generative model
$P(u \mid m)$:

$$
\begin{aligned}
P(u) &= \sum_{m'} P(u \mid m') P(m') \\
     &= \sum_{m'} q(m' \mid C) \frac{P(u \mid m') P(m')}{q(m' \mid C)} \\
     &= \mathbb E_{q(m' \mid C)} \left[ \frac{P(u \mid m') P(m')}{q(m' \mid C)} \right]
\end{aligned}
$$

A proposal function $q$ could enable much more efficient inferences about
speakers' intended meanings, but would do so at the cost of introducing bias.
In our particular case, this proposal function may fail to consider alternative
meanings instantiating the *full*--*empty* contrast, and thus fail to bring
about the explaining-away effect.

This explanation makes a substantial prediction. Such a mechanism of approximate
inverse inference would likely be sensitive to contextual factors. Concretely,
we should be able to design a task (or a priming design) which makes extremely
salient the relevant *full*-*empty* meaning distinction. Increasing the salience
of these alternative meanings should then lead comprehenders to exhibit the
expected explaining-away effect.
<!-- TODO are there examples of the above design for other approx Bayesian accounts? -->

### Non-Bayesian inference

It is also possible that "Bayesian" is not a suitable word to describe the
behavior of comprehenders in our experiment. The time pressure of comprehension
may be too great to support this level of inferential complexity. Instead,
the inference about intended meanings $m$ given an utterance $u$ may simply be
computationally distinct, implemented as a discriminative mapping $P(m \mid u)$.
This mapping may share parameters with speakers' production model, but may
crucially omit some structural features of the production model -- in our case,
the inferential link between evident discourse pressures on the speaker and
the intended meaning of their message.

TODO reference points for this sort of model? cf. Meilin's paper?
...
TODO finish analysis, then summarize in words. Analysis plan:
- model comparison between (both with maximal random effects)
  - base model: `slider_value ~ 0`
  - full model: `slider_value ~ agent_is_given`
