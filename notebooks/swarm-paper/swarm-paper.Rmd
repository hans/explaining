...
```{r 00 regression, echo=FALSE, cache=TRUE}
contrasts(df.00.filtered$agent_is_subject) = c(-1, 1)

model.00 = brm(slider_value ~ agent_is_subject
                            + (agent_is_subject | item_id)
                            + (agent_is_subject | uniqueid),
               data=df.00.filtered %>%
