...
```{r}
model <- lmer(slider_value ~ agent_is_subject * agent_is_concrete + (1|item_id) + (agent_is_subject * agent_is_concrete|uniqueid),
              data=filtered_df)
summary(model)
```
```{r}
...
model.exp00.brm <- brm(slider_value ~ agent_is_subject * agent_is_concrete
                                      + (agent_is_subject|item_id)
                                      + (agent_is_subject * agent_is_concrete|uniqueid),
...
interactions::cat_plot(model.exp00.brm, y.label="Slider response value (0–1)",
                       pred=agent_is_subject, pred.values=c(TRUE, FALSE), pred.labels=c("A (Bees are swarming)", "L (The garden is swarming)"), x.label="Construction",
